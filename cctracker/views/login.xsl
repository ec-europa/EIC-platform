...
<xsl:stylesheet version="1.0"
            ...
            <form class="form-horizontal" action="{$xslt.base-url}login?url={To}" method="post">
              <fieldset>
                ...
                <xsl:if test="not(Hold)">
                  <div class="row" style="margin-top:3em;margin-left:0">
  ...
  <xsl:template match="Login[Production][ECAS]" mode="layout">
    <xsl:attribute name="class">span6 offset3</xsl:attribute>
  </xsl:template>

  <xsl:template match="Login[not(Production)][ECAS]" mode="layout">
  ...
  <xsl:template match="Check">
    <p class="text-warning" style="font-size:133%;line-height: 1.5;text-align:justify" loc="login.merge">Your user profile cannot be located</p>
    <p class="text-warning" style="font-size:133%;line-height: 1.5;text-align:justify;margin-bottom:2em"><span  loc="merge.hint">Please merge</span> <a href="mailto:EASME-SME-COACHING@ec.europa.eu?subject=EU login Casetracker access" style="color:#c09853;">EASME-SME-COACHING@ec.europa.eu</a>.</p>
  </xsl:template>
  
  <xsl:template match="Hold">
    <p class="text-warning" style="font-size:150%;line-height: 1.5;text-align:center;margin-bottom:2em" loc="login.hold">Please come back in a few minutes ...</p>
  </xsl:template>
  ...
  <xsl:template match="ECAS">
    ...
    <div class="span2" style="float:right">
      <xsl:choose>
        <xsl:when test="../To != ''">
          <a class="btn btn-primary" href="login?url={$url}&amp;ecas=init">EU login</a>
        </xsl:when>
        <xsl:otherwise>
