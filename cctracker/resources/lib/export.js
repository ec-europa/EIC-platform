  ...
  function init() {
    if (ExcellentExport) {
      $('#results-export').children('a').first().click(function() {
         return ExcellentExport.excel(this, $('#results').get(0), 'Cases');
       });
       $('#results-export').children('a').eq(1).click(function() {
          return ExcellentExport.csv(this, $('#results').get(0), ",");
        });
       $('#todos-export').children('a').first().click(function() {
       ...
       $('#todos-export').children('a').eq(1).click(function() {
          return ExcellentExport.csv(this, $('[name="todo-kam"]').get(0), ","); //document.getElementsByName("todo-kam")[0]
        });
    }
    if ($.tablesorter) {
