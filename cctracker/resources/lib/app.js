(function () {
  ...
  function init() {
    ...
    $(document).bind('axel-editor-ready',
      function (ev, host) {
        var em = $('[data-target="c-entity-assignment"][data-command="add"]')
        if (em.length > 0) {
          var href = window.location.href.split('#')[0].split('/')
          var idx = href.lastIndexOf('projects')
          if (idx !== -1) {
            if (em.attr('data-with-template').indexOf('project=' + href[idx +1]) == -1) {
              var nw = em.attr('data-with-template') + '&project=' + href[idx +1]
              em.attr('data-with-template', nw)
            }
          }
        }
