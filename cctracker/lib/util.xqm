...
declare function misc:gen_display_name( $refs as element()*, $tag as xs:string ) as element()? {
  misc:gen_display_name( $refs, $tag, ())
};

declare function misc:gen_display_name( $refs as element()*, $tag as xs:string, $separator as xs:string? ) as element()? {
  if ($refs) then
    ...
    let $type :=
      if (ends-with($root, 'y')) then
        ...
        concat($root, 's')
    return
      let $label := display:gen-name-for($type, $refs, $separator, 'en')
      return
        element { $tag } { $label }
  else
    ()
...
declare function local:gen_display_attribute( $refs as element()*, $rg as xs:boolean ) as attribute()? {
  local:gen_display_attribute( $refs, $rg, () )
};

declare function local:gen_display_attribute( $refs as element()*, $rg as xs:boolean, $separator as xs:string? ) as attribute()? {
  if ($refs) then
    let $driver := local-name($refs[1])
    ...
    let $type :=
      if (ends-with($root, 'y')) then
        ...
        concat($root, 's')
    return
      let $label := if ($rg eq true()) then display:gen-name-for-regional-entities( $refs, 'en') else display:gen-name-for($type, $refs, $separator, 'en')
      return
        if ($label) then
...
declare function misc:unreference( $nodes as item()* ) as item()* {
  misc:unreference( $nodes, () )
};

declare function misc:unreference( $nodes as item()*, $separator as xs:string? ) as item()* {
  for $node in $nodes
  return
    typeswitch($node)
      case text()
        ...
        return
          ...
          return
            if (ends-with($tag, 's') and (count($node/*) > 0) and
              ...
              element { $tag }
                {(
                local:gen_display_attribute($node/*, false(), $separator),
                $node/*
                )}
            else if ($tag eq 'RegionalEntityRef') then
              element { $tag }
                {(
                local:gen_display_attribute($node, true(), $separator),
                $node/text()
                )}
              ...
              element { $tag }
                { misc:unreference($node/(attribute()|node()), $separator) }
      default
        return $node
};
...
declare function misc:unreference( $ref as element()?, $tag as xs:string, $lang as xs:string ) as element() {
  element { $tag }
    {(
    local:gen_display_attribute($ref, false()),
    $ref/text()
    )}
};
...
declare function misc:unreference-enterprise( $ref as element()?, $tag as xs:string, $lang as xs:string ) as element() {
  ...
  return
    element { $tag }
      {(
      attribute { '_Display' } { display:gen-enterprise-name($sref, $lang) },
      $sref
      )}
};
...
declare function misc:gen-person-email( $ref as xs:string? ) as xs:string? {
  if ($ref) then
    fn:collection($globals:persons-uri)//Person[Id = $ref]/Contacts/Email/text()
  else 
    ''
};
...
declare function misc:get-extension( $filename as xs:string ) as xs:string
{
  ...
  let $unparsed-extension := lower-case((analyze-string($fn, '\.(\w+)$')//fn:group)[last()])
  return
...
(: ======================================================================
   ...
   ======================================================================
:)
declare function misc:create-collection-lazy ( $base-uri as xs:string, $path as xs:string, $user as xs:string, $group as xs:string ) as xs:string*
{
...
(: =======================================================================
   Remove white spaces at the beginning or the end fo a string
   =======================================================================
:)
declare function misc:trim( $arg as xs:string? )  as xs:string {
