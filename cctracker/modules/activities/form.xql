...
(: ======================================================================
   ...
   ======================================================================
:)
declare function local:gen-challenge-weights( $pid as xs:string, $lang as xs:string, $noedit as xs:boolean, $section as xs:string ) as element()* {
  let $case-no := request:get-parameter('case', '-1')
  let $activity-no := request:get-parameter('activity', '-1')
  let $root := fn:doc($globals:global-information-uri)/GlobalInformation/Description[@Lang eq 'en']/CaseImpact/Sections/Section[Id eq $section]/SectionRoot/text()
  let $targets := fn:collection($globals:projects-uri)/Project[Id eq $pid]/Cases/Case[No eq $case-no]/Activities/Activity[No eq $activity-no]/NeedsAnalysis/Impact/*[local-name(.) eq $root]/*/text()
  let $xtra := if ($noedit) then ';noedit=true' else ''
  let $pairs := 
      for $p in fn:doc($globals:global-information-uri)//GlobalInformation/Description[@Lang = $lang]/CaseImpact/Sections/Section[Id=$section]/SubSections/SubSection[Id = $targets]
...
return
  if ($goal = 'read') then
    ...
    if ($template = 'coaching-assignment') then
      <site:view>
        <site:field Key="weights-vectors" filter="no">
          { local:gen-challenge-weights($project, $lang, true(), '1') }
        </site:field>
        <site:field Key="weights-ideas" filter="no">
          { local:gen-challenge-weights($project, $lang, true(), '2') }
        </site:field>
        <site:field Key="weights-resources" filter="no">
          { local:gen-challenge-weights($project, $lang, true(), '3') }
        </site:field>
        <site:field Key="weights-partners" filter="no">
          { local:gen-challenge-weights($project, $lang, true(), '4') }
        </site:field>
      ...
      <site:view>
        <site:field Key="decl1"><xt:use types="input" param="type=checkbox;name=Evaluation;value=true;noedit=true" label="Evaluation"/></site:field>
        <site:field Key="decl2"><xt:use types="input" param="type=checkbox;name=Personal;value=true;noedit=true" label="Personal"/></site:field>
        <site:field Key="decl3"><xt:use types="input" param="type=checkbox;name=Financial;value=true;noedit=true" label="Financial"/></site:field>
        <site:field Key="decl4"><xt:use types="input" param="type=checkbox;name=Professional;value=true;noedit=true" label="Professional"/></site:field>
      </site:view>
      ...
      let $activity-no := request:get-parameter('activity', '#')
      let $order := fn:collection($globals:projects-uri)/Project[Id eq $project]/Cases/Case[No eq $case-no]/Activities/Activity[No eq $activity-no]/Evaluation/Order[Questionnaire/text() eq 'cctracker-sme-feedback']
      return
        <site:view>
          {
    ...
    if ($template = 'coaching-assignment') then
      <site:view>
        ...
        <site:field Key="assigned-coach">
          { form:gen-person-with-role-selector-service('coach', $lang, " optional;multiple=no;typeahead=yes", "span") }
        </site:field>
        <site:field Key="suggested-coach">
          <xt:use types="constant" param="class=uneditable-input span">Will be available soon</xt:use>
        </site:field>        
        <site:field Key="weights-vectors" filter="no">
          { local:gen-challenge-weights($project, $lang, false(), '1') }
        </site:field>
        <site:field Key="weights-ideas" filter="no">
          { local:gen-challenge-weights($project, $lang, false(), '2') }
        </site:field>
        <site:field Key="weights-resources" filter="no">
          { local:gen-challenge-weights($project, $lang, false(), '3') }
        </site:field>
        <site:field Key="weights-partners" filter="no">
          { local:gen-challenge-weights($project, $lang, false(), '4') }
        </site:field>        
        <site:field Key="date">
    ...
    return
      <site:view>
        <site:field Key="decl1"><xt:use types="input" param="type=checkbox;name=Evaluation;value=true" label="Evaluation"/></site:field>
        <site:field Key="decl2"><xt:use types="input" param="type=checkbox;name=Personal;value=true" label="Personal"/></site:field>
        <site:field Key="decl3"><xt:use types="input" param="type=checkbox;name=Financial;value=true" label="Financial"/></site:field>
        <site:field Key="decl4"><xt:use types="input" param="type=checkbox;name=Professional;value=true" label="Professional"/></site:field>
    ...
    <site:view>
      ...
      <site:field Key="country">
        { form:gen-country-selector($lang, ";multiple=no;typeahead=yes") }
      </site:field>
