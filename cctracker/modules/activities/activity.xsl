...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="Activity">
    <tr>
      ...
      <td class="activity"><xsl:apply-templates select="No"/></td>
  ...
  <xsl:template match="No[parent::Activity][not(ancestor::Activities/@Current)]">
    <a href="{ancestor::Display/@ResourceNo}/activities/{.}"><xsl:value-of select="."/></a>
  </xsl:template>

  <!-- Rendering from an Activity view -->
  <xsl:template match="No[parent::Activity][ancestor::Activities/@Current]">
    <a href="{.}"><xsl:value-of select="."/></a>
  </xsl:template>

  <!-- Rendering from an Activity view -->
  <xsl:template match="No[parent::Activity][. = ancestor::Activities/@Current]" priority="1">
