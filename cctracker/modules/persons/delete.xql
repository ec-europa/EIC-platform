...
declare function local:validate-person-delete( $id as xs:string, $person as element() ) as element()* {
  let $po := fn:collection($globals:projects-uri)//ProjectOfficerRef[. = $id][1]/ancestor::Project/Information/Acronym/text()
  let $kam := fn:collection($globals:projects-uri)//AccountManagerRef[. = $id][1]/ancestor::Project/Information/Acronym/text()
  let $coach := fn:collection($globals:projects-uri)//ResponsibleCoachRef[. = $id][1]/ancestor::Project/Information/Acronym/text()
  let $ref := for $c in fn:collection($globals:projects-uri)//
                (
                AddresseeRef[. = $id][1] |
                SenderRef[. = $id][1] |
                AssignedByRef[. = $id][1] |
              ...
              return $c/ancestor::Project/Information/Acronym/text()
...
declare function local:delete-person( $person as element() ) as element()* {
  ...
  return (
    xmldb:remove(util:collection-name($person), concat($person/Id/text(), ".xml")),
...
let $person-uri := oppidum:path-to-ref-col()
let $id := tokenize($cmd/@trail,'/')[2]
let $person := fn:collection($globals:persons-uri)//Person[Id = $id]
(:let $lang := string($cmd/@lang):)
return
  if ($person) then (: sanity check :)
    if (access:check-omnipotent-user-for('delete', 'Person')) then (: 1st check : authorized user ? :)
