...
<xsl:stylesheet version="1.0"
        ...
        <div class="row login">
          <div class="span7 offset3">
            <form class="form-horizontal" action="{Controller}" method="post">
              ...
              <p><span loc="account.identity1">Vous êtes connecté(e) en tant que</span> "<xsl:value-of select="Username"/>"<xsl:apply-templates select="Realm"/><span loc="account.identity2">et vous devez être</span> "<xsl:value-of select="Name"/>". <xsl:apply-templates select="Groups"/></p>
              <xsl:if test="Realm != 'ECAS'">
  ...
  <xsl:template match="Realm"></xsl:template>

  <xsl:template match="Realm[. = 'ECAS']"><span> using EU Login </span></xsl:template>
  ...
  <xsl:template match="Groups">
    <span loc="account.groups">Groups:</span><xsl:text> </xsl:text><xsl:apply-templates select="Name"/>.
  </xsl:template>

  <xsl:template match="Groups[not(Name)]">
    <span loc="account.nogroup">No group</span>.
  </xsl:template>

  <xsl:template match="Name"><xsl:value-of select="."/>,<xsl:text> </xsl:text>
  </xsl:template>

  <xsl:template match="Name[position() = last()]"><xsl:value-of select="."/>
  </xsl:template>
