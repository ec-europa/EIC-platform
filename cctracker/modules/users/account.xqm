...
declare function account:gen-groups-for-user( $person as element() ) as xs:string* {
  ...
  let $f := fn:doc($globals:global-information-uri)/GlobalInformation/Description[@Lang = 'en']/Functions/Function[Id = $ref]
  return $f/@Group
  )
};
...
(: ======================================================================
   Sends confirmation e-mail to remote user once his/her profile has been 
   added to the application, selecting a specic e-mail template for coach users
   @param $new contains the new values (XQuery update may be executed after) 
   to inject in mail template variables and to send to updated address
   ====================================================================== 
:)
declare function account:confirm-added-remote-login( $remote as element(), $new as element() ) as xs:boolean {
  let $from := media:gen-current-user-email(false())
  let $email :=
    media:render-email(if (exists($remote//FunctionRef[text() eq '4'])) then 'remote-login-added-coach' else 'remote-login-added',
      <vars>
        <var name="Remote_Key">{ $new/Contacts/Name/text() }</var>
      </vars>,
      'en'
      )
  return
    media:send-email('account', $from, $new/Contacts/Mail, $email/Subject, media:message-to-plain-text($email/Message))
};
