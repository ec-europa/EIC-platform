...
let $transition := workflow:pre-check-transition($m, 'Project', $project, (), ())
return
  ...
  (: TODO: implement @Launch protocol calling local:launch-services if needed :)
    ...
    let $errors := workflow:apply-transition($transition, $project, ())
    return
      if ($errors) then
        $errors
      else
        ...
        return
          ...
          let $response := workflow:apply-notification('Case', $success, $transition, $project, (), ())
          return 
