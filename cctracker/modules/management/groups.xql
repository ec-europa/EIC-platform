xquery version "1.0";
(: --------------------------------------
   EIC Coaching application installation
   ...
   @deprecated Since we are using an internal eXist realm groups
   are not managed with eXist any more
...
declare function local:find-users-that-need-repair() {
  for $p in fn:collection('/db/sites/cctracker/persons')//Person/Person
  let $login := $p//Username/text()
  where $login
  order by lower-case($login)
  return
    let $has := if (sm:user-exists(string($login))) then xdb:get-user-groups(string($login)) else '-1' 
  ...
  return
    ...
    return
      if ($has = '-1') then
        ...
        xdb:change-user(string($login), (), $should, ()),
...
return
  <div id="c-groups-results">
    {
    if ($repair = '1') then
      <h1>Réparation des groupes</h1>
    else (
      <h1>Diagnostique des groupes</h1>,
      if (oppidum:get-current-user() = 'admin') then
        <p>Cliquez pour <a href="#" onclick="javascript:$('#c-groups-results').load('management/groups?nrepair=1')">Réparer</a></p>
      else
