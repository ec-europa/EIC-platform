...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:key name="tods" match="Today/Login[(. = 'success') or (. = 'ecas success')]" use="substring(@TS,12,2)" />
  <xsl:key name="todf" match="Today/Login[contains(., 'failure')]" use="substring(@TS,12,2)" />
  <xsl:key name="yes" match="Yesterday/Login" use="substring(@TS,12,2)" />
  <xsl:key name="yess" match="Yesterday/Login[(. = 'success') or (. = 'ecas success')]" use="substring(@TS,12,2)" />
  <xsl:key name="yesf" match="Yesterday/Login[contains(., 'failure')]" use="substring(@TS,12,2)" />
  <xsl:key name="all" match="All/Login" use="substring(@TS,12,2)" />
  <xsl:key name="alls" match="All/Login[(. = 'success') or (. = 'ecas success')]" use="substring(@TS,12,2)" />
  <xsl:key name="allf" match="All/Login[contains(., 'failure')]" use="substring(@TS,12,2)" />
  ...
  <xsl:template match="Logs">
    ...
    <table class="table table-bordered">
      <thead>
        <tr><th>Hours</th><th>Successes / Failures (incl. merge failure)</th></tr>
      </thead>
      <tbody>
    ...
    <table class="table table-bordered">
      <thead>
        <tr><th>Date</th><th>Successes / Failures (incl. merge failure)</th></tr>
      </thead>
      <tbody>
