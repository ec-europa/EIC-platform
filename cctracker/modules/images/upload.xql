    ...
    if (not(doc-available($doc-uri))) then
      ...
      return
        if (xdb:store($col-uri, 'index.xml', $index)) then (
          xdb:set-resource-permissions($col-uri, 'index.xml', $user, $group, util:base-to-integer(0744, 8)),
      ...
      return
        if ($thumb[1] and ($thumb[2] instance of xs:base64Binary)) then (: write thumb :)
          if (xdb:store($col-uri, concat($image-id, '-thumb.', $ext), $thumb[2], $mime-type)) 
            then xdb:set-resource-permissions($col-uri, concat($image-id, '-thumb.', $ext), $user, $group, util:base-to-integer(0774, 8))
