...
(: ------------------------------------------------------------------
   ...
   January 2014 - (c) Copyright may be reserved
   ------------------------------------------------------------------ :)
...
(: ======================================================================
   ...
   ======================================================================
:)
declare function workflow:get-persons-for-role ( $role as xs:string, $project as element(), $case as element()?, $activity as element()? ) as xs:string* {
  let $prefix := substring-before($role, ':')
  let $suffix := substring-after($role, ':')
  return
    if ($prefix eq 'u') then (: targets specific user :)
      fn:collection($globals:persons-uri)//Person[UserProfile/Username = $suffix]/Id/text()  (: FIXME: which realm has to be used in that case :)
      ...
      return
        fn:collection($globals:persons-uri)//Person[UserProfile/Roles/Role/FunctionRef eq $group-ref]/Id/text()
    else  if ($prefix eq 'r') then
      let $func-ref := fn:doc($globals:global-information-uri)/GlobalInformation/Description[@Lang = 'en']/Functions/Function[@Role eq $suffix]/Id/text()
      return
        if ($suffix eq 'region-manager') then
          let $region-entity := $case/ManagingEntity/RegionalEntityRef/text()
          return
            for $role in fn:collection($globals:persons-uri)//Role[(FunctionRef eq $func-ref) and (RegionalEntityRef eq $region-entity)]
          ...
          return
            fn:collection($globals:persons-uri)//Person[UserProfile/Roles/Role[(FunctionRef eq $func-ref) and (ServiceRef eq $service)]]/Id/text()
        else if ($suffix eq 'project-officer') then
          $project/ProjectOfficerRef/text()
        else
          ()
    else
      ()
...
declare function workflow:gen-alert-for-viewing ( $workflow as xs:string, $lang as xs:string, $item as element(), $base as xs:string? ) as element()
{
  <Alert>
    ...
    <ActivityStatus>
      { display:gen-workflow-status-name($workflow, $item/ActivityStatusRef,$lang) }
    </ActivityStatus>,
    $item/Subject,
  ...
  return
    <Annex>
      ...
      if ($item) then
        ...
        <ActivityStatus>
          { display:gen-activity-status-name($item/ActivityStatusRef,$lang) }
        </ActivityStatus>,
        <Sender>
          { display:gen-person-name($item/SenderRef,$lang) }
        </Sender>
  ...
  ) as element()*
{
  if (($workflow eq 'Case') and (count($case//Activity) > 0)) then
    ...
    let $moves :=
      ...
      where access:check-status-change($transition, $project, $case, $activity)
      return $transition
    return
      if ($moves) then
...
(: ======================================================================
   ...
   ======================================================================
:)
declare function local:configure-template( $doc as element(), $project as element(), $case as element()?, $activity as element()? ) as xs:string {
  let $flags :=
    for $f in $doc/Host/Flag
    let $root := string($f/parent::Host/@RootRef)
    let $condition := string($f/parent::Host/@True)
    return
      if ($root) then
        if (access:check-user-can(string($f/@Action), $root, $project, $case, $activity)) then
          concat(string($f/@Name), '=1')
        else
          ()
      else if ($condition) then
        if (util:eval($condition)) then
          concat(string($f/@Name), '=1')
        else
          ()
      else
  ...
  let $params := (
    if (contains($doc/Template/@Param, 'breadcrumbs')) then
      concat('project=', $project/Id, if ($case) then concat('&amp;case=', $case/No) else (), if ($activity) then concat('&amp;activity=', $activity/No) else ())
    else
      ()
    )
...
(: ======================================================================
   ...
   ======================================================================
:)
declare function workflow:assert-rules( $assert as element(), $project as element(), $case as element()?, $activity as element()?, $base as element()? ) as xs:boolean {
...
(: ======================================================================
   ...
   ======================================================================
:)
declare function workflow:validate-document($documents as element(), $doc as element(), $cur-status as xs:string, $project as element(), $case as element()?, $activity as element()? ) as xs:boolean {
  count(
      ...
      if ($cur-status = tokenize($assert/@AtStatus, " ")) then
        ...
        if (count($assert/Tab) > 0 ) then
          if (string($assert/@Rule) eq 'some') then
            if (some $tab in $assert/Tab satisfies workflow:validate-document($documents, $documents/Document[string(@Tab) eq $tab], $cur-status, $project, $case, $activity)) then
              ()
            else (: zero implied documents are displayed so this one too :)
              '0'
          else if (string($assert/@Rule) eq 'all') then
            if (every $tab in $assert/Tab satisfies workflow:validate-document($documents, $documents/Document[string(@Tab) eq $tab], $cur-status, $project, $case, $activity)) then
              ()
...
(: ======================================================================
   ...
   ======================================================================
:)
declare function workflow:gen-information( $workflow as xs:string, $project as element(), $case as element()?, $activity as element()?, $lang as xs:string ) {
  let $target := if ($workflow eq 'Project') then $project else if ($workflow eq 'Case') then $case else $activity
  let $prev-status := $target/StatusHistory/PreviousStatusRef/text()
  let $cur-status := $target/StatusHistory/CurrentStatusRef/text()
  let $status-def := fn:doc($globals:global-information-uri)/GlobalInformation/Description[@Lang = $lang]/WorkflowStatus[@Name eq $workflow]/Status[Id eq $cur-status]
  ...
  return
    <Accordion CurrentStatus="{$cur-status}">
      ...
      return
        ...
        if (((tokenize(string($doc/@AtStatus), " ") = $cur) or ($cur-status = tokenize(string($doc/@AtFinalStatus), " "))) and (workflow:validate-document($documents, $doc, $cur-status, $project, $case, $activity))) then
          ...
          <Template>{string($doc/parent::Documents/@TemplateBaseURL)}{$doc/Template/text()}?goal=read{local:configure-template($doc, $project, $case, $activity)}</Template>,
          if ($actions) then
            <Actions>
              {
              for $a in $actions
              return
                if ($a/@Type = ('update', 'delete')) then
                  ...
                  return
                    if (access:assert-access-rules($rules, $project, $case, $activity)) then
                      if ($verb eq 'update') then
                        <Edit>
                          { $a/@Forward }
                          <Resource>{$doc/Controller/text()}.xml?goal=update</Resource>
                          <Template>{string($doc/parent::Documents/@TemplateBaseURL)}{$doc/Template/text()}?goal=update{local:configure-template($doc, $project, $case, $activity)}</Template>
                        </Edit>
                      else (: assumes delete :)
                        <Delete/>
                  ...
                  workflow:gen-status-change(number($cur-status), $workflow, $project, $case, $activity, string($a/@Id))
...
(: ======================================================================
   ...
   ======================================================================
:)
declare function workflow:validate-transition( $transition as element(), $project as element(), $case as element()?, $activity as element()? ) as xs:string* {
  for $assert in $transition/Assert
  return
    if ($assert/@Error) then
      let $base := util:eval($assert/@Base)
      let $host := if ($activity) then $activity else if ($case) then $case else ()
      return
...
(: ======================================================================
   ...
   ======================================================================
:)
declare function workflow:apply-transition( $transition as element(), $project as element(), $case as element()?, $activity as element()? ) as element()? {
  if ($transition/@To eq 'last()') then
    let $history := if ($activity) then $activity/StatusHistory else if ($case) then $case/StatusHistory else $project/StatusHistory
    let $previous := $history/PreviousStatusRef/text()
    return
      if (exists($previous)) then
        workflow:apply-transition-to($previous, $project, $case, $activity)
      else
        ()
  else
    workflow:apply-transition-to(string($transition/@To), $project, $case, $activity)
};
...
(: ======================================================================
   ...
   ======================================================================
:)
declare function workflow:apply-transition-to( $new-status as xs:string, $project as element(),$case as element()?, $activity as element()? ) as element()? {
  let $history := if ($activity) then $activity/StatusHistory else if ($case) then $case/StatusHistory else $project/StatusHistory
...
(: ======================================================================
   ...
   ======================================================================
:)
declare function workflow:pre-check-transition( $m as xs:string, $type as xs:string, $project as element()?, $case as element()?, $activity as element()? ) as element() {
  let $item := if ($type eq 'Case') then $case else if ($type eq 'Activity') then $activity else $project
      ...
      return
        if ($from ne $cur-status) then
          ...
          return
            ...
            else if (not(access:check-status-change($transition, $project, $case, $activity))) then
              ajax:throw-error('WFSTATUS-NOT-ALLOWED', ())
            else
              (: checks if some document is missing data :)
              let $omissions := workflow:validate-transition($transition, $project, $case, $activity)
              return
...
(: ======================================================================
   ...
   ======================================================================
:)
declare function workflow:gen-recipients( $from as xs:string, $to as xs:string, $target as xs:string, $project as element(), $case as element()?, $activity as element()? ) as element()*
{
  ...
  let $persons :=
    ...
    return workflow:get-persons-for-role($role, $project, $case, $activity)
  return
...
(: ======================================================================
   ...
   ====================================================================== 
:)
declare function workflow:gen-recipient-refs( $rule as xs:string?, $workflow as xs:string?, $project as element(), $case as element()?, $activity as element()? ) as xs:string*
{
  let $persons :=
  ...
  $activity as element()?) as element()
{
  ...
  return alert:notify-transition($transition, $workflow, $project, $case, $activity, $mail/@Template, $mail/Recipients),
  (: 2. Implements @Mail="direct" protocol :)
  if ($transition/@Mail eq 'direct') then (
    (: short-circuit e-mail window, see also 'status' js command :)
    alert:notify-transition($transition, $workflow, $project, $case, $activity, $transition/@Template, $transition/Recipients),
    <success>
      <done/>
      { $success/* }
    </success>
...
declare function workflow:gen-workflow-steps( $workflow as xs:string, $item as element(), $lang as xs:string ) {
  ...
  let $workflow-def := fn:doc($globals:global-information-uri)/GlobalInformation/Description[@Lang = $lang]/WorkflowStatus[@Name eq $workflow]
  return
    <Workflow>
      {(
      $workflow-def/@W,
      $workflow-def/@Offset,
      $workflow-def/@Name,
      for $s in $workflow-def/Status[not(@Deprecated)]
      let $ref := $s/Id/text()
      return
...
(: ======================================================================
   ...
   ====================================================================== 
:)
declare function workflow:gen-source ( $mode as xs:string, $project as element() ) as attribute()? {
  if ($mode eq 'dev') then
    attribute { 'Source' } { 
      let $sel := collection('/db/sites/cctracker/global-information')//Selector[@Name eq 'FundingPrograms']//Option[Id eq $project/Information/Call/FundingProgramRef]/Calls
      let $call := collection('/db/sites/cctracker/global-information')//Selector[@Name = $sel]//Option[Code eq $project/Information/Call/*[ends-with(local-name(.),'CallRef')]/text()]/Name
      let $year := substring($call, 7, 4)
      let $month := substring($call, 4, 2)
      return concat('/exist/rest/db/sites/cctracker/projects/', $year,'/', $month, '/', $project/Id,'/project.xml')
    }
  else
    ()
};
...
(: ======================================================================
   ...
   ======================================================================
:)
declare function workflow:gen-title ( $project as element() ) as xs:string {
  if (string-length($project/Information/Title/text()) > 80) then
    replace(substring($project/Information/Title/text(), 1, 80), "\w+...$", '...')
  else
    $project/Information/Title/text() 
};
declare function workflow:gen-subtitle ( $case as element() ) as xs:string {
  let $benef := $case/../../Information/Beneficiaries/(Coordinator|Partner)[PIC eq $case/PIC]
  return
    if ($benef) then
      $benef/Name
    else
...
declare function workflow:gen-new-activity-tab ( $project as element(), $case as element(), $activity as element()?, $prefixUrl as xs:string ) as element() {
  <Tab Id="new-activity">
    ...
    {
    if (access:check-user-can('create', 'Assignment', $project, $case, ())) then
...
declare function workflow:gen-cases-tab ( $project as element(), $case as element()?, $lang as xs:string ) as element() {
  <Tab Id="cases" Counter="Case">
    <Name loc="workflow.tab.cases">List cases</Name>
    <Heading class="project">
      <Title loc="workflow.title.cases">List of Cases</Title> 
    </Heading>
    <Cases>
      {
      let $cases := cases:gen-cases-for-project($project, $lang)
      let $cur-status := $case/StatusHistory/CurrentStatusRef/text()
      return
        (
        if ($case/No) then
          attribute { 'Current' } { $case/No/text() }
        else
          (),
        if (empty($cases)) then
          <Legend class="c-empty">There is currently no on-going case for this project.</Legend>
        else
          $cases
        )
      }
