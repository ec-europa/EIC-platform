...
(: ------------------------------------------------------------------
   ...
   March 2015 - (c) Copyright may be reserved
   ------------------------------------------------------------------ :)
...
declare function alert:gen-current-user-name() as element()* {
  ...
  let $user := fn:collection($globals:persons-uri)//Person[Id = $uid]
  return (
...
declare function alert:gen-action-status-names( $from as xs:string?, $to as xs:string, $workflow as xs:string ) as element()* {
  let $status := display:gen-workflow-status-name($workflow, $to, 'en')
...
declare function alert:gen-link-to-case-activity( $case as element(), $activity as element()? ) as element()* {
  (
  if ($case) then
    <var name="Link_To_Case">https://casetracker-smei.easme-web.eu/projects/{ $case/../../Id/text() }/cases/{ $case/No/text() }</var>
  else
    (),
  if ($activity) then
    <var name="Link_To_Activity">https://casetracker-smei.easme-web.eu/projects/{ $case/../../Id/text() }/cases/{ $case/No/text() }/activities/{ $activity/No/text() }</var>
  else
    ()
  )
};
...
declare function alert:gen-case-activity-title( $case as element(), $activity as element()? ) as element()* {
  let $project := $case/../..
  let $client := $project/Information/Beneficiaries/(Coordinator|Partner)[PIC eq $case/PIC]/Name/text()
  return (
    <var name="Project_Acronym">{ $project/Information/Acronym/text() }</var>,
...
declare function alert:check-user-has-login( $ref as element()? ) as xs:string {
  if ($ref) then 
    let $login := normalize-space(fn:collection($globals:persons-uri)//Person[Id = $ref/text()]/UserProfile/Username) (: FIXME: become inconsistent since using ECAS we cannot assert it (?) :)
    return 
      if ($login and sm:user-exists($login)) then '' else '-nologin'
  else
    ''
};
...
{
  let $send-to := workflow:gen-recipient-refs($recipients, (), $project, $case, $activity)
  let $send-cc := (workflow:gen-recipient-refs($recipients/@CC, (), $project, $case, $activity), $alert/CC/text())
  return
    let $to :=
    ...
    return
      <Report Total="{ $done }" Message="{ string-join($res, ', ') }">
        ...
        if ($done > 0) then (: succeeded for at list one recipient => archives it :)
          let $home := if ($activity) then $activity else if ($case) then $case else $project
  ...
  return 
    if (($recipients/@Max eq '1') and ($recipients/@Key) and ($host/Alerts/Alert[Key eq tokenize($recipients/@Key, ' ')[1]])) then
      ...
      return
        ...
        let $report := alert:apply-recipients($recipients, 'workflow', $project, $case, $activity, $alert, $from, $wf-to, $wf-from)
        return (
