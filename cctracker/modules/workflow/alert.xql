...
(: ======================================================================
   ...
   ======================================================================
:)
declare function local:prefill-message( $context as xs:string, $workflow as xs:string, $project as element(), $case as element(), $activity as element()? ) as element() {
  let $spec := fn:doc($globals:application-uri)/Application/Messages/Email[@Context eq $context]
  let $recipients := $spec/Recipients
  let $send-to := workflow:gen-recipient-refs($recipients/text(), $workflow, $project, $case, $activity)
  (: cc: automatically added when posting :)
  let $alert := email:render-alert(if ($spec/@Template) then $spec/@Template else 'Unkown', 'en', $project, $case, $activity)
...
(: ======================================================================
   ...
   ======================================================================
:)
declare function local:prefill-notification( $workflow as xs:string, $project as element(), $case as element(), $activity as element()? ) as element() {
  ...
  let $alert := email:render-alert($template, 'en', $project, $case, $activity, $extra-vars)
  return (: by default Date and Sender prefilled directly from form.xql :)
    <Alert>
      {
      misc:rename-element('Sender', $alert/From),
      workflow:gen-recipients($wf-from, $wf-to, $workflow, $project, $case, $activity),
      $alert/(Subject | Message) 
      } 
    </Alert>
};
  ...
  if (request:get-parameter('next', ()) = 'redirect') then
    ...
    return
      if ($workflow eq 'Case') then
        ...
        ajax:report-success-redirect('ACTION-ALERT-CREATED', (),
          concat($cmd/@base-url, string-join(tokenize($cmd/@trail,'/')[position() <= 6],'/')))
  else
...
(: ======================================================================
   ...
   ======================================================================
:)
declare function local:add-message( $context as xs:string, $workflow as xs:string, $project as element(), $case as element()?, $activity as element()?, $submitted as element(), $lang as xs:string ) as element()*
{
  let $host := if ($activity) then $activity else if ($case) then $case else $project
  let $spec := fn:doc($globals:application-uri)/Application/Messages/Email[@Context eq $context]
  let $send-cc := workflow:gen-recipient-refs($spec/Recipients/@CC, $workflow, $project, $case, $activity)
  return
  ...
  ) as element()*
{
  ...
  return
    if (local-name($saved) eq 'Alert') then
      local:report-success($workflow, $lang, $saved, $parent/(No|Id))
...
let $item := if ($activity-no) then $activity else if ($case-no) then $case else $project
return
  if ($item) then
    if ($m = 'POST') then
        ...
        if ($doc-id = 'alerts') then
          ...
          return
            ...
            if ($from eq 'SME-Agreement') then
              ...
              local:add-message($from, $target, $project, $case, $activity, $submitted, $lang)
            else
      ...
      return
        if ($goal = 'init') then
          if ($from = 'status') then
            local:prefill-notification($target, $project, $case, $activity)
          else if ($from = 'SME-Agreement') then
            local:prefill-sme-agreement-email($case, $activity)
          else if ($from = 'FundingDecision') then
            local:prefill-message($from, $target, $project, $case, $activity)
          else
            email:render-alert(concat(lower-case($target), '-spontaneous-alert'), 'en', $project, $case, $activity)
