...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:include href="../../modules/cases/case.xsl"/>
  ...
  <xsl:template match="success" priority="1">
    <success>
  ...
  <xsl:template match="payload" priority="1">
    <payload>
  ...
  <xsl:template match="Display[@Mode = 'workflow']" priority="1">
    <site:view skin="workflow">
      ...
      <site:title>
        ...
        <h1><xsl:apply-templates select="Cartouche/Subtitle"/></h1>
      </site:title>
      <site:timeline>
  ...
  <xsl:template match="Title[parent::Cartouche]">
    <xsl:value-of select="."/><xsl:apply-templates select="@Source" mode="title"/>
  </xsl:template>
  ...
  <xsl:template match="Step[@Display='step']">
    ...
    <li>
      <span>
        ...
        <xsl:value-of select="/Display/Dictionary/WorkflowStatus/Status[Id/text() = $num]/Name"/>
      </span>
  ...
  <xsl:template match="Step[@Display='state'][@StartDate !='']">
    ...
    <li>
      <span>
        ...
        <xsl:value-of select="/Display/Dictionary/WorkflowStatus/Status[Id = $num]/Name"/>
      </span>
  ...
  <xsl:template match="Document">
    <div class="accordion-group c-documents" data-command="view" data-target="c-editor-{@Id}"
      ...
      <div class="accordion-heading c-{@Status} {@class}">
        <span class="c-document-menu c-menu-scope"><xsl:apply-templates select="Actions/*[not(ChangeStatus) or local-name(.) != 'Spawn']"/></span>
  ...
  <xsl:template match="Spawn[parent::Actions]">
    <button class="btn btn-primary" data-command="confirm" data-src="{/Display/@ResourceNo}/{Controller}">
      <xsl:choose>
        <xsl:when test="@loc"><xsl:value-of select="@loc"/></xsl:when>
        <xsl:otherwise>Create coaching activity</xsl:otherwise>
      </xsl:choose>
    </button>
  </xsl:template>
      ...
      <a tabindex="-1" href="#" data-action="{@Action}">
        ...
        <xsl:choose>
          ...
          <xsl:when test="number(parent::ChangeStatus/@Status) &lt; number($to)">
            <xsl:apply-templates select="@Intent"/>Advance to “<xsl:value-of select="/Display/Dictionary/WorkflowStatus/Status[Id = $to]/Name"/>”
          </xsl:when>
          <xsl:otherwise>
            <xsl:apply-templates select="@Intent"/>Return to “<xsl:value-of select="/Display/Dictionary/WorkflowStatus/Status[Id = $to]/Name"/>”
          </xsl:otherwise>
        </xsl:choose>
      </a>
    </li>
  ...
  <xsl:template match="Sender[@Mode = 'batch']" mode="alert" priority="2">case tracker batch
  </xsl:template>

  <!-- <xsl:template match="Sender[. != ''][../From]" mode="alert"><xsl:value-of select="../From"/> (triggered by <xsl:value-of select="."/>)
  </xsl:template> -->

  <xsl:template match="Sender[. = ''][not(../To)]" mode="alert" priority="1"><i>unregistered user</i>
  </xsl:template>

  <xsl:template match="Sender[@Mode = 'auto']" mode="alert" priority="2">case tracker reminder
  </xsl:template>
  ...
  <xsl:template match="Cases">
    <table>
      ...
      <thead>
        <xsl:if test="Add">
          <xsl:attribute name="data-command">header</xsl:attribute>
          <xsl:attribute name="data-target">case-editor</xsl:attribute>
        </xsl:if>
        <tr>
          <th loc="term.no">No</th>
          <th loc="term.beneficiary">Bénéficiaire</th>  
          <th loc="term.managingEntity">Entité EEN</th>
          <th loc="term.kam">KAM</th>
          <th loc="term.creationDate">Date de création</th>
          <th loc="term.status">Statut</th>
          <th>Linked Coaching activities</th>
        </tr>
      </thead>
      <tbody>
  ...
  <xsl:template match="Cases[Case]" mode="class">
    <xsl:attribute name="class">table table-bordered</xsl:attribute>
  </xsl:template>
  
  <xsl:template match="Cases[not(Case)]" mode="class">
    <xsl:attribute name="class">table table-bordered c-empty</xsl:attribute>
  </xsl:template>
