...
declare function local:create-enterprise( $cmd as element(), $data as element(), $lang as xs:string ) as element() {
  ...
  return
    if (empty($errors)) then
      ...
      return (
        update insert enterprise:gen-enterprise-for-writing((),$data,$newkey) into fn:doc($globals:enterprises-uri)/Enterprises,
  ...
  if ($m = 'POST') then
    ...
    return
      if ($cmd/@action = 'add') then
        if (access:check-omnipotent-user-for('create', 'Enterprise')) then
          local:create-enterprise($cmd, $data, $lang)
        else
          oppidum:throw-error('FORBIDDEN', ())
      else
        if (access:check-omnipotent-user-for('update', 'Enterprise')) then
