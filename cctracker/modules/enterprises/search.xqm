...
declare function local:gen-enterprise-sample ( $e as element(), $lang as xs:string ) as element() {
  <Enterprise>
    ...
    <Persons>
      {
      string-join(for $p in fn:collection($globals:persons-uri)//Person[EnterpriseRef eq $e/Id/text()]
...
declare function search:fetch-enterprises ( $request as element() ) as element() {
  let $omni := access:check-omnipotent-user-for('update', 'Enterprise')
  return
    <Results>
      <Enterprises>
        {
...
declare function local:fetch-some-enterprises ( $filter as element() ) as element()*
{
  ...
  return
    ...
    where (empty($enterprise) or $e/Id = $enterprise)
      ...
      and (empty($person) or fn:collection($globals:persons-uri)//Person[(Id = $person) and (EnterpriseRef eq $e/Id/text())])
    order by $e/Name
    return
      local:gen-enterprise-sample($e, 'en')
};
