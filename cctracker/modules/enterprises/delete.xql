...
declare function local:validate-enterprise-delete( $id as xs:string ) as element()* {
  ...
  let $person := fn:collection($globals:persons-uri)//Person[EnterpriseRef = $id][1]/Name
  return
...
return
  if ($enterprise) then (: sanity check :)
    if (access:check-omnipotent-user-for('delete', 'Enterprise')) then (: 1st check : authorized user ? :)
