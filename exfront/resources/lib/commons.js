...
(function () {
  ...
  function _makeTableCommand ( name, encodeRowFunc, tableRowModel ) {
    var kommand = new Function(
                        ['key', 'node'],
                        'this.spec = $(node); this.table = this.spec.attr("id"); this.spec.bind("click",$.proxy(this,"handleAction")); this.modals={};this.config={};this.configure();'
                  );
    kommand.prototype = (function (encodeRowFunc, tableRowModel) {
      ...
      return {
        ...
        configure : function () {
          var params = this.spec.attr("data-table-configure");
          if (params) {
            if (params.indexOf('analytics') !== -1) {
              if (!this.config.analytics) {
                this.config.analytics = this.spec.attr("data-analytics-controller") || 'analytics';
                this.config.uuid = this.spec.attr("data-analytics-uuid") || $($.parseXML($axel('#' + this.spec.attr("data-editor")).xml())).find('UUID').text();
              }
            }
        ...
        handleAction : function (ev) {
          ...
          if ((hotspot === 'A') || (hotspot === 'BUTTON')) {
            ...
            if (editor) { // shows corresponding modal editor
              ...
              callback(uid, key, target, this.config.uuid);
