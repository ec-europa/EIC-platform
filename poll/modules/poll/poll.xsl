...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xt="http://ns.inria.org/xtiger" xmlns:site="http://oppidoc.com/oppidum/site" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns="http://www.w3.org/1999/xhtml">
  ...
  <!-- ***** Configuration *****  -->
  <xsl:param name="xslt.context">poll</xsl:param>
  <!-- application context name for filtering with site:conditional -->
  <xsl:param name="xslt.default-variable">on</xsl:param>
  <!-- when 'on' generates Prefill/@DefaultVariable to pre-fill field default value -->
  <xsl:param name="xslt.likert-class"/>
  <!-- use ";class=something" syntax to add class to likert fields -->
  ...
  <xsl:template match="/Poll">
    <html xmlns="http://www.w3.org/1999/xhtml" xmlns:xt="http://ns.inria.org/xtiger" xmlns:site="http://oppidoc.com/oppidum/site">
      <head>
        ...
        <xsl:comment>XTiger XML template generated with "poll.xsl" from EIC POLL application</xsl:comment>
        <xsl:text>
    </xsl:text>
  ...
  <!-- site:conditional overlay with @context (see also supergrid.xsl rule) -->
  <xsl:template match="site:conditional[@context][$xslt.context != @context]" priority="20"> </xsl:template>
  ...
  <xsl:template match="Prefill" mode="component">
    ...
    <xt:component name="t_{$key}">
      <div class="control-group">
        ...
        <div class="controls">
          <xt:use param="class=span12 a-control" types="input">
            <xsl:apply-templates select="@DefaultVariable"/>
          </xt:use>
        </div>
      </div>
    </xt:component>
  ...
  <xsl:template match="@DefaultVariable"/>

  <xsl:template match="@DefaultVariable[$xslt.default-variable = 'on']">
    <site:field Key="{.}" force="true" type="var">
      <xsl:value-of select="."/>
    </site:field>
  </xsl:template>
  ...
  <xsl:template match="CommentatedQuestion" mode="component">
    ...
    <xsl:variable name="CommentLabel">
      <xsl:value-of select="@CommentLabel"/>
    </xsl:variable>
    <xt:component name="t_{$Label}">
      <div class="row comm-quest" style="margin-left:0">
        <xsl:apply-templates select="Question"/>
        <div class="span12" style="margin-left:0">
          <div class="control-group">
            <label class="control-label a-gap0">
              <xsl:value-of select="$CommentLabel"/>
            </label>
            <div class="controls">
  ...
  <xsl:template match="Question">
    ...
    <xsl:variable name="prefix">
      <xsl:value-of select="/Poll/Plugins/*[contains(@Keys, $key)]/@Prefix"/>
    </xsl:variable>
    <div class="row">
      <div class="span12">
  ...
  <xsl:template match="Likert">
    <xt:use types="choice" param="appearance=full;multiple=no{$xslt.likert-class}">
      <xsl:attribute name="values">
        <xsl:apply-templates select="Option" mode="Likert-id"/>
      </xsl:attribute>
      <xsl:attribute name="i18n">
        <xsl:apply-templates select="Option" mode="Likert-name"/>
      </xsl:attribute>
    </xt:use>
  </xsl:template>

  <xsl:template match="Option" mode="Likert-id">
    <xsl:value-of select="Id"/>
    <xsl:text> </xsl:text>
  </xsl:template>

  <xsl:template match="Option[position() = last()]" mode="Likert-id">
    <xsl:value-of select="Id"/>
  </xsl:template>

  <xsl:template match="Option" mode="Likert-name">
    <xsl:value-of select="Name"/>
    <xsl:text> </xsl:text>
  </xsl:template>

  <xsl:template match="Name[position() = last()]" mode="Likert-name">
    <xsl:value-of select="Name"/>
  </xsl:template>
  ...
  <!--     MultiText Plugin     -->
  <!-- ************************ -->
  <xsl:template match="Comments | MultiText">
