...
declare function local:gen-and-save-form( $name as xs:string, $base-url as xs:string, $spec as element() ) {
  ...
  let $sg := concat('file://', system:get-exist-home(), '/webapp/projets/poll/modules/poll/poll.xsl')
  return
...
return
  if (empty($errors)) then
    ...
    return
        if (exists($local:usecret) and exists($local:psecret) and xdb:exists-user($local:usecret)) then 
          <poll>
            {
