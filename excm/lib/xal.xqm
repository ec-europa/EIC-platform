...
module namespace xal = "http://oppidoc.com/ns/xal";

declare namespace xdb = "http://exist-db.org/xquery/xmldb";
import module namespace oppidum = "http://oppidoc.com/oppidum/util" at "../../oppidum/lib/util.xqm";
import module namespace database = "http://oppidoc.com/ns/database" at "database.xqm";
import module namespace cache = "http://oppidoc.com/ns/cache" at "cache.xqm";
  ...
  if (local-name($fragment) eq 'MaxLength') then
    ...
    return
      if ($length > $limit) then
        oppidum:throw-error(($fragment/@Error, $xal-spec/@Error)[1], ($length, $length - $limit, $limit))
      else
        ()
    ...
    if ($fragment/text() eq 'true') then
      ...
      oppidum:throw-error(($fragment/@Error, $xal-spec/@Error)[1], $fragment/@Param)
...
declare function local:apply-xal-create( $subject as element()?, $xal-spec as element() ) as element() {
  if ($xal-spec/@Debug eq 'on') then
    ...
    (),
  let $db := if (exists($xal-spec/@Db-uri)) then $xal-spec/@Db-uri else oppidum:get-command()/@db
...
declare function xal:apply-updates( $subject as item()*, $object as item()*, $spec as element()? ) as element() {
  ...
  if (every $fragment in $spec/XALAction satisfies $fragment/@Type = $xal:xal-actions) then (: sanity check :) 
    ...
    let $res :=
      ...
      return
        if ((count($pivot) > 1) and $type ne 'delete') then
