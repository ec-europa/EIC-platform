...
module namespace cache = "http://oppidoc.com/ns/cache";
...
{
  globals:doc('cache')//Entry[@Id eq $name][@lang eq $lang][not(@Dirty)]
};
...
declare function cache:update( $name as xs:string, $lang as xs:string, $values as xs:string,  $i18n as xs:string?) {
  if (globals:doc-available('cache')) then
    let $found := globals:doc('cache')//Entry[@Id eq $name][@lang eq $lang]
    let $fresh :=
    ...
    return
      if ($found) then
        ...
        update insert $fresh into globals:doc('cache')/Cache
