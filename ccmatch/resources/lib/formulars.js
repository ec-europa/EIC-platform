(function () {
  ...
  function init() {
    $axel.setup({ bundlesPath : 'static/ccmatch/lib/axel/bundles', tabGroupNavigation: true });
    ...
    $('#x-display').bind('click', function() { 
      ...
      t = xtiger.cross.loadDocument(tplurl); 
      // $('body xt\\:use', t).attr('types','t_simulation')
      $axel('#x-simulator').transform(t); 
      $('#c-editor-errors').removeClass('af-validation-failed'); 
      $axel.command.install(document, target);
      $axel.binding.install(document, target); 
      });
    $('#x-validate').bind('click', function() { $axel.binding.validate($axel('#x-simulator'), 'c-editor-errors', document, '.af-label, label') });
