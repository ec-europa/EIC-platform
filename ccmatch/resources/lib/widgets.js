...
(function ($axel) {
    ...
    DeleteCommand.prototype = {
      ...
      successCb : function (response, status, xhr) {
        ...
        if (loc) { // one shot protocol
          ...
          cmd = $axel.oppidum.getCommand(xhr); // TODO: $axel.oppidum.decodeMessage(cmd);
          if (cmd.doc) {
  ...
  (function ($axel) {
    ...
    OpenCommand.prototype = {
      execute : function (ev) {
        var editor, ed, wset, evt, initEvt,
            link = this.spec.attr('data-open-link'),
            load = this.spec.attr('data-load-link'),
...
(function ($axel) {
  ...
  function genRadar( host, data, config ) {
    var plot = [[]], 
        pourcent = config.format === '%';
    // conversion to {axis, value} format
    data.forEach(function(p, i) {
      var v = parseFloat(p.Score),
          d = { axis : p.For,
                value : isNaN(v) ? 0 : pourcent ? Math.round(v * 10) / 1000 : v - config.delta
              };
      if (isNaN(v)) {
        d.miss = true;
      }
    ...
    successCb : function (response, status, xhr) {
      ...
      if (response.Summary) {
        genRadar(this.spec.get(0), response.Summary.Axis,
          ...
          w: 300,
          h: 300,
          maxValue: parseInt(response.Settings.Max) || 1,
          levels: parseInt(response.Settings.Levels) || 4,
          ExtraWidthX: 200,
          TranslateX: 100,
          delta : parseFloat(response.Settings.Delta) || 0,
          format : response.Settings.Format || '%',
          });
      } else {
        this.spec.children('svg').remove();
      }
      if (response.Detail) { // FIXME: data-injector="Detail" + hash of hash protocol
        $.each( response.Detail, function( okey, ovalue ) {
          $.each( ovalue, function( ikey, ivalue ) {
            $('#' + okey + '-' + ikey).text(ivalue);
          });
        });
      }
      $('*[data-toggle="tab"][href="#' + this.spec.attr('data-event-target') + '"]')
