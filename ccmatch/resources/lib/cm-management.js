(function () {
  ...
  var GENUSER = {
    Name : {
      ...
      'resource' : 'management/users/$_',
      'template' : 'templates/coach/contact?goal=update&user=$_',
    ...
    Login : {
      ...
      'resource' : 'management/accounts/$_',
    ...
    Access : {
      ...
      'resource' : 'management/accounts/$_?goal=create'
    },
    Admin : {
      'yes' : '<b>yes</b> (<a>no<a>)',
      'no' : '<b>no</b> (<a>yes</a>)',
  ...
  var GENIMPORT = {
    Name : { // or RemoteName
      yes : '*',
      editor : 'cm-update-person-editor',
      resource : 'management/users/$_?table=import&persists=$!',
      template : 'templates/coach/contact?goal=update&user=$_'
    },
    RemoteName : { // or Name
      yes : '.',
    ...
    Login : {
      ...
      no : '---',
      editor : 'cm-update-account-editor',
      resource : 'management/accounts/$_?table=import&persists=$!'
    },
    Access : { // or NoAccess
      yes : 'yes',
      no : '<a>create</a>',
      editor: 'cm-create-account-editor',
      resource : 'management/accounts/$_?goal=create&table=import&persists=$!'
    },    
    NoAccess : { // or Access
      yes : '---',
      no : '---'
  ...
  function itemDeleteDone (ev, editor, source, xhr) {
    ...
    $('#cm-' + key + '-editor-modal').modal('hide'); // should make display modal appear
    if (key === 'update-person') {
      id = $('Value', xhr.responseXML).text();
      $axel.command.getCommand('user-table', 'cm-user-results').removeRowById(id)
    }
  }
  ...
  function init() {
    ...
    $('#cm-update-person-editor').bind('axel-delete-done', itemDeleteDone);
    // login pane
