...
<xsl:stylesheet version="1.0"
  ...
  <!-- Installs spinning wheel with given Id 
      TODO: factorize and move to commons.xsl ? -->
  <xsl:template match="SpinningWheel">
    <p class="cm-busy" style="display:none;margin-left:300px">
  ...
  <!--  Full page statistics UI -->
  <xsl:template match="/Stats[not(@Embedded)]">
    <site:view skin="stats">
      <xsl:apply-templates select="Window"/>
      <site:content>
        <xsl:apply-templates select="Formular"/>
        <xsl:apply-templates select="SpinningWheel"/>
        <h2 id="with-sample">
          <span>Data set contains </span><xsl:text> </xsl:text>
          <span id="dataset-nb">0</span><span> coaches</span>
        </h2>
  ...
  <!--  Embedded statistics UI -->
  <xsl:template match="/Stats[@Embedded]">
    <div class="span12">
      <xsl:apply-templates select="Formular"/>
      <xsl:apply-templates select="SpinningWheel"/>
      <h2 id="with-sample">
  ...
  <xsl:template match="Command" mode="form">
    <form id="c-{@Form}-form" enctype="multipart/form-data" accept-charset="UTF-8"
      action="{@base-url}{@Action}" method="post" target="_blank" style="display:none">
  ...
  <xsl:template match="Command[@Name = 'export']">
    <xsl:variable name="offset"><xsl:if test="@Offset"><xsl:value-of select="concat(' offset', @Offset)"/></xsl:if></xsl:variable>
    <div class="span{@W}{$offset}">
      <xsl:value-of select="@Prompt"/><a data-command="export" data-target="editor" data-action="{@base-url}{@Action}" href="#"><xsl:value-of select="."/></a>
    </div>
  </xsl:template>
  ...
  <xsl:template match="Command[@Name = 'stats']">
    <xsl:variable name="offset"><xsl:if test="@Offset"><xsl:value-of select="concat(' offset', @Offset)"/></xsl:if></xsl:variable>
    <div class="span{@W}{$offset}">
      <button id="c-stats-submit" class="btn btn-primary" data-command="stats" data-src="{@base-url}{@Action}">
  ...
  <xsl:template match="Chart[Composition]">
    <div style="display:none" data-set="{Set}">
      <xsl:call-template name="chart-class"/>
  ...
  <xsl:template name="chart-class">
    <xsl:attribute name="class">
      <xsl:choose>
        <xsl:when test="@Library">chart</xsl:when>
        <xsl:otherwise>chart d3pure</xsl:otherwise>
      </xsl:choose>
    </xsl:attribute>
  </xsl:template>

  <xsl:template match="@Library">
    <xsl:attribute name="data-library"><xsl:value-of select="."/></xsl:attribute>
  </xsl:template>
  ...
  <xsl:template match="Width">
    <xsl:attribute name="data-width"><xsl:value-of select="."/></xsl:attribute>
  </xsl:template>
