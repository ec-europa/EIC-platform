...
declare function local:validate-user( $login as xs:string, $mail as xs:string ) as element() {
  let $user := fn:collection($globals:persons-uri)//Person[UserProfile/Username eq normalize-space($login)]
  return
    if (($login ne '') and ($mail ne '')) then
      if ($user) then
        if ($user/Information/Contacts/Email) then
          if ($user/Information/Contacts/Email eq normalize-space($mail)) then
            $user
          else
        ...
        ajax:throw-error('VALIDATION-NO-PROFILE', concat($login, ' / ', $mail))
    else
  ...
  return (
    if (account:send-new-password($user, $new-pwd, $to, true())) then (
      util:catch('*', 
                 ...
                 ajax:throw-error('CHANGE-PASSWORD-FAILURE', $to)
                 ),
  ...
  return
    if ($change and $check and ($change eq $check)) then
      if (local:validate-password-submission($change)) then (
        ...
        <Changed><Username>{normalize-space($user/UserProfile/Username)}</Username></Changed>
        )
      else
...
return
    ...
    if ($cmd/@action = 'forgotten') then (: tunnel to generate a new password :)
      ...
      let $user := fn:collection($globals:persons-uri)//Person[UserProfile/Username = xdb:get-current-user()]
      return
        if ($user) then
          if ($m eq 'GET') then
            local:gen-ask-pwd-model($user)
