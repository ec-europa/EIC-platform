...
<!-- Oppidoc Supergrid application
     ...
     Utility transformation to generates XTiger XML templates with <site:field Key="name">
     extension points to insert the final AXEL input fields. It takes an XML form grid
     specification as input as found in the formulars folder.

     Dependencies:
     - the generated grid layout is based on Boostrap
     - you must also load [resources/css/]forms.css after Bootstrap
     - your epilogue MUST implement a site:field function to substitute the extension points
     - you MUST provide a form.xql script to actually generate the <site:field Key="name">
       input fields in the application template generation pipeline
...
<xsl:stylesheet version="1.0"
  ...
  <xsl:param name="xslt.base-root"></xsl:param>
  <xsl:param name="xslt.goal">test</xsl:param>

  <xsl:include href="search-mask.xsl"/>
  <!-- <xsl:include href="poll.xsl"/> -->
  <xsl:include href="../../lib/shared.xsl"/>
  <xsl:include href="tabular.xsl"/>
      ...
      <xt:head version="1.1" templateVersion="1.0" label="{@Tag}">
        ...
        <xsl:if test="$xslt.goal = 'test'">
          <xt:component name="t_fake_field">
            <xt:use types="choice" values="choix1 choix2 choix3" param="placeholder=test;class=span12 a-control"/>
          </xt:component>
  ...
  <xsl:template match="Field[$xslt.goal = 'save']" priority="1">
    <xsl:variable name="key"><xsl:value-of select="@Key"/></xsl:variable>
    <xsl:variable name="offset"><xsl:if test="@Offset">offset<xsl:value-of select="concat(@Offset,' ')"/></xsl:if></xsl:variable>
    <xsl:variable name="W"><xsl:choose><xsl:when test="@W"><xsl:value-of select="@W"/></xsl:when><xsl:otherwise>12</xsl:otherwise></xsl:choose></xsl:variable>
    <xsl:variable name="gap"><xsl:if test="floor(@Gap) = @Gap"><xsl:value-of select="@Gap"/></xsl:if></xsl:variable>
  ...
  <xsl:template match="Field[. = ''][$xslt.goal = 'save']" priority="2">
    <xsl:variable name="key"><xsl:value-of select="@Key"/></xsl:variable>
    <xsl:variable name="pos"><xsl:value-of select="count(./preceding::Field[string(@Key) = $key])"/></xsl:variable>
    <div>
      <xsl:choose>
  ...
  <xsl:template match="Cell[not(@Gap)]">
    <xsl:variable name="W"><xsl:choose><xsl:when test="@W"><xsl:value-of select="@W"/></xsl:when><xsl:otherwise>12</xsl:otherwise></xsl:choose></xsl:variable>
    <xsl:variable name="offset"><xsl:if test="@Offset">offset<xsl:value-of select="concat(@Offset,' ')"/></xsl:if></xsl:variable>
    <xsl:variable name="class"><xsl:if test="@Class"><xsl:text> </xsl:text><xsl:value-of select="@Class"/></xsl:if></xsl:variable>
    <div class="{$offset}span{$W}{$class}">
  ...
  <xsl:template match="Cell[@Tag]" priority="1">
  ...
  <!-- DECOMMISSIONED (too marginal): see coach-registration.xml -->
  <xsl:template match="Link">
    ...
    <xsl:text> </xsl:text><xsl:value-of select="@Post"/>
  </xsl:template>
...
param="constant_media=image;image_base={$xslt.base-url}{substring-after(@Base,'/')};noimage={$xslt.base-url}static/ccmatch/images/identity.png;class=img-polaroid"/>
  </xsl:template>
  ...
  <xsl:template match="Selector">
    ...
    <xt:use label="{//Field[@Key = $key]/@Tag}" types="choice"  param="select2_dropdownAutoWidth=true;select2_width=off;class=span12 a-control;filter=select2;multiple=no;typeahead=yes">
      <xsl:attribute name="values">
        <xsl:apply-templates select="document('xmldb:exist:///db/sites/ccmatch/global-information/global-information.xml')//Selector[@Name = $name]//Option" mode="values"/>
      </xsl:attribute>
      <xsl:attribute name="i18n">
        <xsl:apply-templates select="document('xmldb:exist:///db/sites/ccmatch/global-information/global-information.xml')//Selector[@Name = $name]//Option" mode="i18n"/>
      </xsl:attribute>
      <xsl:if test="@Default">
  ...
  <xsl:template match="Option" mode="values"><xsl:value-of select="Id/text()"/><xsl:text> </xsl:text>
  </xsl:template>

  <xsl:template match="Option[last()]" mode="values"><xsl:value-of select="Id/text()"/>
  </xsl:template>

  <xsl:template match="Option" mode="i18n">
  ...
  <xsl:template match="Option[last()]" mode="i18n">
    <xsl:call-template name="replace-string">
      <xsl:with-param name="text" select="Name/text()"/>
      <xsl:with-param name="replace" select="' '"/>
      <xsl:with-param name="with" select="'\ '"/>
  ...
  <!-- DECOMMISSIONED: use data-binding='required' instead (see coach-registration.xml) -->
  <xsl:template name="Radio">
    ...
    <xt:use label="{//Field[@Key = $key]/@Tag}" param="type=radio;filter=event;class={$span} a-control{$klass}{$required};value=on" types="input"/>
  </xsl:template>
  ...
  <!-- FIXME: limitation - non sharable Keys  -->
  <!-- Abandoned because of difficult CSS integration ! -->
  <!-- <xsl:template match="Text">
    <xsl:param name="key"></xsl:param>
    <xsl:variable name="required">
      <xsl:if test="//Form/Bindings/Require[contains(@Keys, $key)]">;required=true</xsl:if>
    </xsl:variable>
    <xt:use types="text" label="{//Field[@Key = $key]/@Tag}" param="type=textarea;placeholder=empty;shape=parent;class=sg-textarea span a-control;filter=optional{$required}"/>
  </xsl:template> -->

  <!-- FIXME: limitation - non sharable Keys  -->
  <!-- TODO: handle maxlength attribute, handle min-height -->
  <xsl:template match="Text">
    <xsl:param name="key"></xsl:param>
    <xsl:variable name="required">
      <xsl:if test="//Form/Bindings/Require[contains(@Keys, $key)]">;required=true</xsl:if>
    </xsl:variable>
    <xt:use types="input" label="{//Field[@Key = $key]/@Tag}" param="type=textarea;placeholder=empty;class=sg-textarea span a-control;filter=optional{$required}"/>
  </xsl:template>
  ...
  <xsl:template match="Plain[@Type = 'text']">
    ...
    <xsl:attribute name="signature">plain</xsl:attribute>
    <xsl:attribute name="class">sg-plain</xsl:attribute>
    <xt:use types="text" label="{//Field[@Key = $key]/@Tag}" param="filter=event;type=textarea;shape=parent-30px;placeholder=clear;class=sg-plain"><xsl:value-of select="."/></xt:use>
  </xsl:template>

  <xsl:template match="Plain[@Type = 'number']">
    <xsl:param name="key"></xsl:param>
    <xsl:attribute name="signature">plain</xsl:attribute>
    <xt:use types="input" label="{//Field[@Key = $key]/@Tag}" param="filter=event;type=text;class=sg-plain"></xt:use>
  </xsl:template>
    ...
    <xt:component name="t_{@Id}">
      <div id="{@Id}-modal" aria-hidden="true" role="dialog" tabindex="-1" class="modal hide fade" style="width:{@Width};margin-left:-{$margin}px"
        ...
        <div class="modal-header">
            <xsl:if test="not(@Dismiss) or (@Dismiss != 'none')">
        ...
        <div class="modal-footer c-menu-scope">
          ...
          <xsl:apply-templates select="Footer" mode="modal"/>
          <xsl:if test="not(Footer)">
            <button class="btn btn-primary" data-command="save c-inhibit" data-target="{@Id}" data-validation-output="{@Id}-errors" data-validation-label="label" loc="{$label4save}">
  ...
  <xsl:template match="Footer" mode="modal">
    <xsl:apply-templates select="Button" mode="modal"/>
  </xsl:template>
  
  <!-- Custom button for modal window footer -->
