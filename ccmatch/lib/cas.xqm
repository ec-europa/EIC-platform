...
declare function cas:get-user-profile-in-realm($realm-name as xs:string) as element()? {
  fn:collection($globals:persons-uri)//Person[UserProfile/Remote eq oppidum:get-current-user() and UserProfile/Remote/@Name = $realm-name]
};
...
declare function cas:get-user-profile() as element()? {
  ...
  return
    if (exists($remote) and exists($remote/key) and exists($remote/@name)) then
      fn:collection($globals:persons-uri)//Person[UserProfile/Remote eq oppidum:get-current-user() and UserProfile/Remote/@Name = $remote/@name]
    else
...
declare function cas:create-profile($realm-name as xs:string, $remote as xs:string, $goto as xs:string?) {
  ...
  return
    ...
    return
      ...
      let $col-uri := misc:create-collection-lazy ($globals:persons-uri, $col, 'admin', 'users', 'rwxrwxr--')
