...
{
  (:let $prefix := substring($filename, 1, local:index-of-string($filename, $extension) -1 ) :)
  ...
  return
    if (($filtered[2] instance of xs:base64Binary)
      ...
      (
      custom:set-owner-group-permissions(concat($col-uri, '/', $filename), $user, $group, $perms),
          ...
          return
            if (not(empty($thumb)) and $thumb[1] and ($thumb[2] instance of xs:base64Binary)) then
              if (xdb:store($col-uri, $thumbname, $thumb[2], $mime-type)) then
                custom:set-owner-group-permissions(concat($col-uri, '/', $thumbname), $user, $group, $perms)
              else
