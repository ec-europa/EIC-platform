...
<xsl:stylesheet version="1.0"
  ...
  <!-- off Tab for scaffolding incomplete functions -->
  <xsl:template match="Tab[@Display = 'off']" mode="nav">
    <li>
      <xsl:copy-of select="@class"/>
      <a style="color:gray" onclick="javascript:return false">
        <i>
          <xsl:copy-of select="Name/@loc"/>
          <xsl:value-of select="Name"/>
        </i>
  ...
  <!-- TODO: factorize in commons.xsl micro-format section -->
  ...
  <xsl:template match="Save" mode="Modal">
    <button class="btn btn-primary" data-command="save c-inhibit" data-target="{ancestor::Modal/@Id}" data-save-flags="disableOnSave silentErrors" data-validation-label="label">
      <xsl:if test="not(NoValidation)">
        <xsl:attribute name="data-validation-output"><xsl:value-of select="ancestor::Modal/@Id"/>-errors</xsl:attribute>
      </xsl:if>
      <xsl:copy-of select="@*[starts-with(local-name(.), 'data-' )]"/>
  ...
  <xsl:template match="Formular[parent::Search]">
    <form class="ecl-form" action="" onsubmit="return false;">
  ...
  <xsl:template match="Button">
    <button>
      <xsl:attribute name="class">
        <xsl:choose>
          <xsl:when test="@class">ecl-button <xsl:value-of select="@class"/></xsl:when>
          <xsl:otherwise>ecl-button ecl-button--primary</xsl:otherwise>
        </xsl:choose>
      </xsl:attribute>
      <xsl:copy-of select="@*[starts-with(local-name(.), 'data-' )]"/>      
  ...
  <xsl:template name="formular-imp">
    ...
    <div class="row commands">
      <xsl:if test="Commands/*[@Position = 'aside']">
        <div style="float:left" >
          <xsl:apply-templates select="Commands/*[@Position = 'aside']"/>
        </div>
      </xsl:if>
      <div style="float:right" >
        <xsl:apply-templates select="Commands/*[not(@Position) or @Position != 'aside']"/>
      </div>
