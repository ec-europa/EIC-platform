...
declare function local:process-result($end-point as element(), $result as element(), $start as xs:string) as element() {
  let $rules := $end-point/Result[@Http eq $result/@statusCode]
  let $realm := string($end-point/parent::Realm/@Name)
  return
    if (empty($rules)) then
      oppidum:throw-error('CAS-INVALID-INIT-NO-RULES', ($realm, $result/@statusCode))
    else
      let $to-apply := local:get-rules-from-context($rules, $result)
      return
        if (empty($to-apply/(Failure | Success | Combine))) then
          oppidum:throw-error('CAS-INVALID-INIT-NO-VALID-RULES', ($realm, $result/@statusCode))
        else if (count($to-apply/Failure) > 0 and count($to-apply/Success) > 0) then
          oppidum:throw-error('CAS-INVALID-INIT-MISMATCHING-RULES', ($realm, $result/@statusCode))
...
declare function cas:get-user-profile-in-realm($realm-name as xs:string) as element()? {
  let $remote := oppidum:get-current-user()
  let $all := session:get-attribute('cas-res') (: superset of cas-user :)
  let $model := fn:doc(oppidum:path-to-config('security.xml'))//Realm[@Name eq $realm-name]
  return
    let $person-remote := fn:collection($globals:persons-uri)//Person[UserProfile/Remote eq oppidum:get-current-user() and UserProfile/Remote/@Name = $realm-name]
    return
      if ($person-remote) then
        $person-remote
      else (: fallback and lazy merge if match occurs :)
        let $mail-match := fn:collection($globals:persons-uri)//Person[lower-case(UserProfile/Email) eq lower-case($all/email) and UserProfile/Email/@Name = $realm-name]
        return
          if ($mail-match) then
            (
            let $remote-user:= $model//Variable[Name eq 'RemoteUser']/Expression
            return update insert util:eval($remote-user) into $mail-match/UserProfile,
            $mail-match
            )[last()]
          else
            ()
...
declare function cas:get-predefined-user-profile-in-realm($realm-name as xs:string) as element()? {
  fn:collection($globals:remotes-uri)//Remote[Realm = $realm-name][lower-case(Key/text()) eq lower-case(oppidum:get-current-user())]
};
...
declare function cas:has-user-profile-in-realm($realm-name as xs:string) as xs:boolean {
  count(cas:get-user-profile-in-realm($realm-name)) > 0
};

declare function cas:get-user-profile() as element()? {
  let $remote := session:get-attribute('cas-user')
  return
    if (exists($remote) and exists($remote/key) and exists($remote/@name)) then
      fn:collection($globals:persons-uri)//Person[lower-case(Information/Contacts/Email) eq lower-case($all/email)]
    else
      fn:collection($globals:persons-uri)//Person[UserProfile/Username eq xdb:get-current-user()]
};
...
(: ======================================================================
   ...
   ======================================================================
:)
declare function cas:has-predefined-profile-in-realm($realm-name) as xs:boolean {
  count(cas:get-predefined-user-profile-in-realm($realm-name)) > 0
};
...
(: ======================================================================
   Log on to eXist and store user identity picked from the CAS 
   into the new session

   @param $all optional complete user record as returned by cas validation
   service, not available when invoked after cas:merge
   ====================================================================== 
:)
  ...
  return
    if ($user-profile and exists($val)) then
      ...
      return
        ...
        (:TODO: update value $user-profile/../Information/Contacts/Email with email from 'cas-res' ? :)
      )
