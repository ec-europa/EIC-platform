...
declare function local:gen_display_attribute( $refs as element()* ) as attribute()? {
  if ($refs) then
    let $driver := local-name($refs[1])
    return
      if ($driver eq 'EnterpriseRef') then
        attribute { '_Display' } 
          { 
          string-join(for $r in $refs return custom:gen-enterprise-name($r, 'en'), ', ') 
          }
      else
        ...
        return
          ...
          return
            if ($node/@_Unref) then
              ...
              element { $tag }
                {(
                  attribute { '_Display' } { custom:gen-person-name($node/text(), 'en') },
                  $node/text()
                )}
                
            else if (ends-with($tag, 'Ref') or (ends-with($tag,'Country'))) then
              element { $tag }
                {(
                local:gen_display_attribute($node),
                $node/text()
...
declare function misc:gen-current-person-name() as xs:string {
  ...
  let $name := custom:gen-person-name($uid, 'en')
  return
  ...
  if ($person) then
    ...
    return
      element { node-name($person) }
        {(
        attribute { '_Display' } { custom:gen-person-name($ref, $lang) },
        $ref
        )}
  else
    ()
...
declare function misc:get-extension( $filename as xs:string ) as xs:string
{
  ...
  let $unparsed-extension := lower-case((tokenize($fn, '\.'))[last()])
  return
...
(: ======================================================================
   Returns the ordinal of the day of the date in parameter
   Built upon on the basis of the week day that is both in the past and a
   sunday
   ====================================================================== 
:)
declare function misc:day-of-week ( $date as xs:anyAtomicType? ) as xs:integer? {
  if (empty($date)) then
    ()
  else
    1 + ((days-from-duration(xs:date( substring($date, 1, 10)) - xs:date('1901-01-06')) - 1) mod 7)
};

declare function misc:net-working-days ( $start as xs:anyAtomicType?, $end as xs:anyAtomicType? ) as xs:integer? {
  if(empty($start) or empty($end)) then
    ()
  else
    if ($start > $end) then
      -misc:net-working-days( $end, $start ) (: still positive return :)
    else
      let $ord-start := misc:day-of-week($start)
      let $ord-end := misc:day-of-week($end)
      let $dist := xs:integer(days-from-duration(xs:dateTime($end) - xs:dateTime($start)) div 7) * 5 (: count of complete weeks times 5 working days :)
      let $mod := days-from-duration(xs:dateTime($end) - xs:dateTime($start)) mod 7
      let $mod2 := days-from-duration(xs:date( substring($end, 1, 10) ) - xs:date( substring($start, 1, 10) )) mod 7
      let $disth := if ($mod < $mod2)(: if ($mod > 0 and xs:time( substring( $end, 12, 18 )) < xs:time( substring( $start,12,18 ) )):) then 1 else 0  
      return
        if ($ord-start <= $ord-end) then (: manually computes the number of days that remain :)
          $dist + max(((min((($ord-end + 1), 6)) - $ord-start), 0)) - 1 - $disth
        else
          $dist + min((($ord-end + 6) - min(($ord-start, 6)), 5)) - 1 -$disth
};
