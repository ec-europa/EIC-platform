...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="Editor">
    <div class="row-fluid" style="margin-top: 0px;margin-bottom: 0px">
      <div id="c-editor-{@Id}-menu" class="c-menu-scope span12 offset9">
        <xsl:apply-templates select="Action" mode="editor"/>
      </div>
    </div>
    <div class="row-fluid c-documents">
  ...
  <xsl:template match="Action[Controller]" mode="editor"> 
  ...
  <xsl:template match="Action[Goto]" mode="editor"> 
    <a class="ecl-button ecl-button--primary"
      href="{ Goto }"><xsl:value-of select="Label"/></a>
  </xsl:template>
