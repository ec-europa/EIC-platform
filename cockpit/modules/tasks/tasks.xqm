...
declare function tasks:add-task($context as xs:string, $name as xs:string, $eid  as xs:string, $priority as xs:integer) as element() {
  let $tasks := fn:doc($globals:tasks-uri)/Tasks  
  let $date := string(current-time())
  let $task :=  <Task context="{ $context }" name="{ $name }" enterprise="{ $eid }" priority="{ $priority }" submission-date="{ $date }"/>
  return
    if (not(exists($tasks/Task[@enterprise eq $eid][@context eq "EICCommunity"][@name eq $name]))) then
  ...
  try 
  {
    ...
    return 
      if (exists($current-task/Task)) then
        ...
        let $service := globals:doc('services-uri')//Consumers/Service[Id eq 'community']
