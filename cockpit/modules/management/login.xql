...
declare function local:gen-login-log-all( $tag as xs:string, $resource as xs:string ) as element() {
  element { $tag } {
    let $logins := fn:doc('/db/debug/login.xml')//(Login[(@User ne '') and (. = ('ecas success', 'success', 'ie') or (. = ('ecas failure', 'failure') and $resource = 'access'))] | Hold | Logout)
    let $count := count($logins)
    return
    (
      attribute { 'UniCount' } { $count },
      attribute { 'Days' } { days-from-duration(xs:dateTime($logins[last()]/@TS) - xs:dateTime($logins[1]/@TS)) }, 
      for $l in fn:doc('/db/debug/login.xml')//(Login[(@User ne '') and (. = ('ecas success', 'success', 'ie') or (. = ('ecas failure', 'failure') and $resource = 'access'))] | Hold | Logout)
