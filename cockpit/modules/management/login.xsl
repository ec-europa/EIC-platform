...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="Login[contains(., 'guest access -')]">
    <p style="color:orange"><xsl:value-of select="@User"/> at <xsl:value-of select="substring(@TS, 12, 5)"/><xsl:apply-templates select="@UA"/> (<xsl:value-of select="."/>)</p>
  </xsl:template>
  ...
  <xsl:template match="Login[. = 'ecas success']">
    <p><xsl:value-of select="@User"/> at <xsl:value-of select="substring(@TS, 12, 5)"/><xsl:apply-templates select="@UA"/> (ecas<xsl:apply-templates select="@Email"/>)</p>
  </xsl:template>

  <xsl:template match="Login[. = 'ecas guest access']">
    <p style="color:orange"><xsl:value-of select="@User"/> at <xsl:value-of select="substring(@TS, 12, 5)"/><xsl:apply-templates select="@UA"/> (guest access<xsl:apply-templates select="@Email"/>)</p>
  </xsl:template>
  ...
  <xsl:template match="@Email"><xsl:text> - </xsl:text><xsl:value-of select="."/>
  </xsl:template>
</xsl:stylesheet>
