...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
  ...
  <!--///////////////////////
    Exceptions (no blender)
    ///////////////////////-->

  <!-- tag often used in event application registration form not to be confounded with rich text Title -->
  <xsl:template match="Title[parent::Event]" priority="1">
    <xsl:copy-of select="."/>
  </xsl:template>
  
  <!-- You need to add rules for any Tag with a content model 
       that needs to be rended as HTML for static display 
       by the 'html' plugin.... 
  ...
  <xsl:template match="CompanyDescription">
    <CompanyDescription>
      <xsl:apply-templates select="*"/>
    </CompanyDescription>
  </xsl:template>
