...
let $log-user := if (globals:doc('log-file-uri')/Logs/@Hold[. ne '']) then oppidum:get-current-user() else ()
return
  <Redirected>
    {
    ...
    if ($log-user) then
      ...
      return
        update insert <Logout User="{$log-user}" TS="{$ts}"/> into globals:doc('log-file-uri')/Logs
    else
      ()
    }
  </Redirected>
